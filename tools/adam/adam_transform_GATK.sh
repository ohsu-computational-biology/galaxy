...
HDFS_RI_PATH_FILE=$7
HDFS_BQSR_PATH_FILE=$8

#Remove // if it exists in the path coming from DTS
SEARCH='//'
HDFS_BAM_PATH_FILE=${HDFS_BAM_PATH_FILE/'//'/'/'}
...
iferr=$(adam-submit --conf spark.shuffle.service.enable=true --master yarn-client transform $HDFS_ADAM_PATH_FILE $HDFS_SORT_PATH_FILE  -sort_reads 2>&1)
fnc_check_error $? "Transform Sorting Function Succeeded on Hadoop/Spark/Adam" "$iferr" 0

iferr=$(adam-submit --conf spark.shuffle.service.enable=true --master yarn-client transform $HDFS_SORT_PATH_FILE $HDFS_MKDUP_PATH_FILE  -mark_duplicate_reads 2>&1)
...
iferr=$(adam-submit --conf spark.shuffle.service.enable=true --master yarn-client transform $HDFS_MKDUP_PATH_FILE $HDFS_BQSR_PATH_FILE -recalibrate_base_qualities -known_snps $HDFS_VAR_PATH_FILE 2>&1)
fnc_check_error $? "Recalibrating Base Qualities Function Succeeded on Hadoop/Spark/Adam" "$iferr" 0

iferr=$(adam-submit --conf spark.shuffle.service.enable=true --master yarn-client transform $HDFS_BQSR_PATH_FILE $HDFS_RI_PATH_FILE -realign_indels 2>&1)
